...
def read_reply_lines(max_lines=N_FPGA, max_nop=2000):
    lines = []
    current = []
...
def run_one_case(name, lines, use_hw_reset=False):
    ...
    send_case(lines)
    actual_all = read_reply_lines(N_FPGA)
    drain_done()

    expected_n, expected_all = expected_lines(lines)
    actual_n = actual_all[:len(lines)]

    print("[fpga output: first {} receivers]".format(len(lines)))
    for line in actual_n:
        print(line_to_text(line))

    ok_main = actual_n == expected_n
    ok_tail = actual_all[len(lines):] == expected_all[len(lines):]

    if ok_main and ok_tail:
        print("PASS")
    else:
        print("FAIL")
        print("[expected: first {} receivers]".format(len(lines)))
        for line in expected_n:
            print(line_to_text(line))

        if not ok_tail:
            print("[tail mismatch: receivers {}..{}]".format(len(lines) + 1, N_FPGA))
            for i in range(len(lines), N_FPGA):
                if actual_all[i] != expected_all[i]:
                    print(
                        "receiver {}: actual={} expected={}".format(
                            i + 1,
                            line_to_text(actual_all[i]),
                            line_to_text(expected_all[i]),
                        )
                    )
...
def main():
    for name, lines in TEST_CASES:
        run_one_case(name, lines, use_hw_reset=True)
